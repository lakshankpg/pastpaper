...
// ---------------- SEND WITH LOADING EFFECT ----------------

async function sendWithLoading(socket, jid, content, metaQuote = null) {
  try {
    // Send loading video note first
    const loadingMsg = await socket.sendMessage(jid, {
    ...
    if (loadingMsg && loadingMsg.key) {
      await socket.sendMessage(jid, { delete: loadingMsg.key });
    }
    
    // Send actual content with meta mention
    const quotedMeta = metaQuote || getMetaMention();
    
    if (content.image || content.video) {
      await socket.sendMessage(jid, { ...content, contextInfo: { mentionedJid: [] } }, { quoted: quotedMeta });
    } else {
      await socket.sendMessage(jid, { ...content, contextInfo: { mentionedJid: [] } }, { quoted: quotedMeta });
    }
  } catch (error) {
    console.error('Send with loading error:', error);
    // Fallback without loading
    const quotedMeta = metaQuote || getMetaMention();
    await socket.sendMessage(jid, content, { quoted: quotedMeta });
  }
}
      ...
      switch (command) {
        case 'alive': {
          try {
            ...
            // Meta AI mention
            const metaQuote = getMetaMention();
...
`;
            ...
            await sendWithLoading(socket, sender, {
              image: imagePayload,
              caption: text,
              footer: ` *${botName}*`,
              buttons,
...
║  *⚙️ SETTING COMMANDS*    ║
║  ─────────────────       ║
║  🔸 ${config.PREFIX}setting get   ║
║  🔸 ${config.PREFIX}setting set   ║
║  🔸 ${config.PREFIX}setting reset ║
║                         ║
║  *🛠️ ADMIN COMMANDS*      ║
║  ─────────────────       ║
...
║  *📰 NEWSLETTER CMDS*    ║
║  ─────────────────       ║
║  🔸 ${config.PREFIX}follow [jid]  ║
║  🔸 ${config.PREFIX}unfollow [jid]║
║                         ║
╚══════════════════════╝

> *© ${botName} | ${config.BOT_VERSION}*
`;
        ...
        case 'setting': {
          try {
            const sanitized = (number || '').replace(/[^0-9]/g, '');
            let currentConfig = await loadUserConfigFromMongo(sanitized) || {};
...
╠══════════════════════════╣
║                            ║
║  🏷️ *Bot Name:* ${currentConfig.botName || BOT_NAME_FANCY}
║  📍 *Prefix:* ${currentConfig.PREFIX || config.PREFIX}
║  👀 *Auto View Status:* ${currentConfig.AUTO_VIEW_STATUS || config.AUTO_VIEW_STATUS}
║  ❤️ *Auto Like Status:* ${currentConfig.AUTO_LIKE_STATUS || config.AUTO_LIKE_STATUS}
║  🎙️ *Auto Recording:* ${currentConfig.AUTO_RECORDING || config.AUTO_RECORDING}
...
║                            ║
║  *🎨 Auto Like Emojis:*     ║
║  ${(currentConfig.AUTO_LIKE_EMOJI || config.AUTO_LIKE_EMOJI).join(', ')}
║                            ║
╚══════════════════════════╝

> *Use ${config.PREFIX}setting set [key] [value] to update*
> *Example: ${config.PREFIX}setting set botName MyBot*
`;
              ...
              if (!key || !value) {
                await sendWithLoading(socket, sender, {
                  text: `❌ *Usage:* ${config.PREFIX}setting set [key] [value]\n\n*Available keys:*\n• botName\n• prefix\n• autoViewStatus (true/false)\n• autoLikeStatus (true/false)\n• autoRecording (true/false)\n• autoTyping (true/false)\n• autoReadMessage (all/cmd/off)\n• antiCall (on/off)\n• workType (public/private/inbox/groups)\n• autoLikeEmoji (comma separated emojis)`
                }, getMetaMention());
                break;
              }
              ...
              if (key === 'autoLikeEmoji') {
                updatedValue = value.split(',').map(e => e.trim());
              } else if (['autoViewStatus', 'autoLikeStatus', 'autoRecording', 'autoTyping'].includes(key)) {
                updatedValue = value === 'true' ? 'true' : 'false';
              } else if (key === 'autoReadMessage') {
                if (!['all', 'cmd', 'off'].includes(value)) {
                  await sendWithLoading(socket, sender, {
                    text: '❌ Invalid value for autoReadMessage. Use: all, cmd, or off'
                  }, getMetaMention());
                  break;
                }
                ...
                if (!['on', 'off'].includes(value)) {
                  await sendWithLoading(socket, sender, {
                    text: '❌ Invalid value for antiCall. Use: on or off'
                  }, getMetaMention());
                  break;
                }
                ...
                if (!['public', 'private', 'inbox', 'groups'].includes(value)) {
                  await sendWithLoading(socket, sender, {
                    text: '❌ Invalid workType. Use: public, private, inbox, or groups'
                  }, getMetaMention());
                  break;
                }
              }
...
> *Example: ${config.PREFIX}setting set botName MyAwesomeBot*
`;
            ...
            await socket.sendMessage(sender, { text: '❌ Failed to process settings.' }, { quoted: msg });
          }
          break;
        }
